...
An external test process imports the resolved addresses and configuration as a standard Aspire testing builder. The same test can therefore build, start, wait for resources, and create clients through `DistributedApplication` in either mode:

```csharp
IDistributedApplicationTestingBuilder testBuilder = mode switch
{
    "apphost" => await DistributedApplicationTestingBuilder.CreateAsync<Projects.EShop_E2E_AppHost>(),
    "compose" => DockerComposeDeploymentTestingBuilder
        .CreateFromEnvironment<Projects.EShop_E2E_AppHost>()
};

await using var app = await testBuilder.BuildAsync();
