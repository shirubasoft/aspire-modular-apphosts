...
- deploy the AppHost through Aspire's Docker Compose environment and run the tests against that deployment.
...
```csharp
compose
    .WithTestEndpoint("catalog-api", catalog.Api.GetEndpoint("http"))
    .WithTestEndpoint("orders-api", orders.Api.GetEndpoint("http"))
    .WithTestValue("orders-api-key", ordersApiKey.Resource);
```

An external test process imports the resolved addresses and configuration without parsing Compose YAML or knowing Aspire's generated port variable names:

```csharp
var deployment = AspireDeploymentTestConfiguration.LoadFromEnvironment();
using var orders = deployment.CreateHttpClient("orders-api");
var apiKey = deployment.GetValue("orders-api-key");
```

Set `ASPIRE_TEST_CONFIGURATION_FILE` to the `.env.<environment>` file generated by `aspire deploy`. That file can contain resolved secrets and is ignored by this repository; treat it as sensitive CI workspace data and do not upload it as an artifact.
